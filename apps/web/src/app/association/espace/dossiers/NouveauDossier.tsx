...
export function NouveauDossier({ dispositifs }: { dispositifs: Dispositif[] }) {
  ...
  if (!ouvert) {
    return (
      <button type="button" onClick={() => setOuvert(true)} className="rounded-md bg-[#1F6A4E] px-4 py-2.5 text-sm font-medium text-white hover:bg-[#185540]">
        Nouveau dossier
      </button>
    );
  }
  ...
  return (
    <form onSubmit={soumettre} className="grid gap-3 rounded-md border border-[#DDD8CC] bg-white p-5 sm:grid-cols-2">
      <label className="flex flex-col gap-1 text-sm sm:col-span-2">
        <span className="font-medium">Dispositif connu (facultatif)</span>
        ...
        <span className="text-xs text-[#5C6B63]">Un dispositif connu apporte sa liste de pièces exigées.</span>
      </label>
      ...
      <label className="flex flex-col gap-1 text-sm">
        <span className="font-medium">Intitulé du dossier</span>
