...
/**
 * La fiche d'une action : peu de champs, et seulement ceux qui servent. Les
 * chiffres du bas sont exactement ceux qu'un financeur demande.
 */
export function FicheAction({ action, onFermer }: { action: ActionAssociation | null; onFermer: () => void }) {
  const router = useRouter();
  const [v, setV] = useState<Valeurs>(depuis(action));
  ...
  async function enregistrer(e: FormEvent) {
    ...
    try {
      if (action) await appel(`/association/actions/${action.id}`, { method: 'PATCH', body });
  ...
  async function supprimer() {
    if (!action) return;
    if (!window.confirm(`Retirer « ${action.intitule} » de mes actions ?`)) return;
    setEnCours(true);
    try {
      await appel(`/association/actions/${action.id}`, { method: 'DELETE' });
      router.refresh();
      onFermer();
    } catch (err) {
  ...
  return (
    <form onSubmit={enregistrer} className="flex flex-col gap-4 rounded-2xl border-2 border-[#4F46E5] bg-white p-5 sm:p-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-extrabold text-[#1D1B5C]">{action ? action.intitule : 'Une nouvelle action'}</h3>
      ...
      <label className="flex flex-col gap-1 text-sm">
        <span className="font-bold text-[#1D1B5C]">Le nom de l&apos;action *</span>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button type="submit" disabled={enCours} className="rounded-xl bg-[#4F46E5] px-5 py-3 text-base font-bold text-white hover:bg-[#4338CA] disabled:opacity-60">
          {enCours ? 'Enregistrement…' : action ? 'Enregistrer' : 'Ajouter cette action'}
        </button>
        {action ? (
