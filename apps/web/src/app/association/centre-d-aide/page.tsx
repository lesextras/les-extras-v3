...
export const metadata: Metadata = {
  ...
  alternates: { canonical: '/centre-d-aide' },
};
...
export default function CentreAide() {
  return (
    <>
      <p className="text-sm font-extrabold uppercase tracking-[0.14em] text-[#6B6A8A]">Aide</p>
      ...
      <div className="mt-7 grid gap-4 sm:grid-cols-3">
        {RACCOURCIS.map((r) => (
          <Link
            key={r.href}
            href={r.href}
            className="rounded-2xl border border-[#E6E4F3] bg-white p-5 no-underline transition hover:border-[#4F46E5]"
          >
            <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#ECEBFC] text-[#4F46E5]" aria-hidden="true">
              <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d={r.icone} />
              </svg>
            </span>
            <span className="mt-3 block text-[17px] font-extrabold text-[#1D1B5C]">{r.titre}</span>
            <span className="mt-1 block text-[14px] leading-relaxed text-[#3B3A66]">{r.detail}</span>
          </Link>
        ))}
      </div>
      ...
      {SECTIONS.map((s) => (
        <section key={s.titre} className="mt-10">
          <h2 className="text-2xl font-extrabold tracking-tight text-[#1D1B5C] sm:text-3xl">{s.titre}</h2>
          <div className="mt-4 space-y-3">
            {s.questions.map((q) => (
              <details key={q.q} className="group rounded-2xl border border-[#E6E4F3] bg-white p-5 open:border-[#C7C4F2]">
                <summary className="cursor-pointer list-none text-[17px] font-extrabold text-[#1D1B5C] marker:hidden">
                  {q.q}
                </summary>
                <p className="mt-2 max-w-[70ch] leading-relaxed text-[#3B3A66]">{q.r}</p>
                {q.lien ? (
                  <Link href={q.lien.href} className="mt-3 inline-block font-bold text-[#4F46E5] underline underline-offset-4">
                    {q.lien.libelle}
                  </Link>
                ) : null}
              </details>
      ...
      <section className="mt-10 rounded-[24px] border-2 border-[#F3B0C2] bg-[#FDE7EC] p-7 text-center sm:p-9">
        <h2 className="text-2xl font-extrabold leading-tight tracking-tight text-[#8A1B3D] sm:text-3xl">
          Ta question n&apos;est pas là ?
        </h2>
        <p className="mx-auto mt-3 max-w-[52ch] leading-relaxed text-[#8A1B3D]/85">
          Écris-nous en quelques lignes : on répond nous-mêmes, il n&apos;y a pas de service client automatique derrière.
        </p>
        <Link
          href="/nous-contacter"
          className="mt-6 inline-flex rounded-xl bg-[#C42B57] px-6 py-3.5 text-base font-extrabold text-white no-underline transition hover:bg-[#8A1B3D]"
        >
          Nous écrire
        </Link>
      </section>
...
const RACCOURCIS = [
  ...
  },
  {
    ...
    detail: "Particulier, association ou académie : trois portes, une par situation.",
...
const SECTIONS = [
  {
    ...
    questions: [
      ...
      {
        q: "Je n'ai pas encore d'association. Je peux quand même m'en servir ?",
        r: "Oui : c'est l'espace particulier. Tu ouvres un compte à ton nom, tu suis le chemin, et tu créeras ta structure en route. L'espace se transformera le jour où elle existera.",
        lien: { libelle: 'Ouvrir un espace particulier', href: '/inscription?type=particulier' },
      },
      {
  ...
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        lien: { libelle: "Droits d'accès", href: '/droits-acces' },
      },
      {
