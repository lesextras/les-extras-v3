...
//  - la note n'apparaît pas s'il n'y a aucun avis. Un « 0 ★ » sur un compte
//    neuf est un faux signal négatif.
...
export function IndicateursCompte({
  ...
  const aDesNotes = notes && notes.count > 0 && notes.average != null;
  if (!solde && !aDesNotes) return null;
      ...
      {aDesNotes ? (
        ...
        >
          ...
          <span className="sr-only">
            note moyenne sur {notes.count} avis reçus
          </span>
        </Link>
      ) : null}
    </div>
