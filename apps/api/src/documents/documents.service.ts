...
export class DocumentsService {
  ...
  async formation(
    ...
    // Une inscription ANNULÉE ne donne droit à rien. Le contrôle manquait
    // complètement : on pouvait éditer une « attestation d'assiduité » pour
    // quelqu'un qui s'était désinscrit. Un document daté, tamponné et faux
    // est pire que pas de document du tout — c'est lui qui part au financeur.
    if (inscription.status === 'CANCELLED') {
      throw new BadRequestException(
        "Cette inscription a été annulée : aucune attestation ni certificat ne peut être délivré.",
      );
    }

    // Et une attestation d'ASSIDUITÉ suppose une présence constatée. Sans
    // émargement, le PDF affichait « 0 présence » sous un titre qui affirme
    // le contraire.
    if (genre === 'attestation' && (inscription.emargements?.length ?? 0) === 0) {
      throw new BadRequestException(
        "Aucune présence n'est enregistrée pour cette inscription : l'attestation d'assiduité ne peut pas être établie tant que la feuille d'émargement est vide.",
      );
    }
