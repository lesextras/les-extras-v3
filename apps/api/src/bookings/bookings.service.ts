...
import { BAREME, CommunityService, prestationsTermineesDe } from '../community/community.service';
...
export class BookingsService {
  ...
  private async recompenserParrainage(accountId: string) {
    const compte = await this.prisma.account.findUnique({
      where: { id: accountId },
      select: { parrainAccountId: true, name: true, type: true },
    });
    ...
    // Le montant depend de CE QU'EST le filleul, pas de qui l'a parraine :
    // un etablissement qui entre fait vivre la place de marche des deux
    // cotes, un intervenant n'en alimente qu'un. Voir BAREME.
    const montantParrainage =
      compte.type === 'ESTABLISHMENT'
        ? BAREME.PARRAINAGE_ETABLISSEMENT
        : BAREME.PARRAINAGE;
