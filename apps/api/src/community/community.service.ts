...
export const BAREME = {
  ...
  PARRAINAGE: 40,
  /**
   * UN FILLEUL ÉTABLISSEMENT VAUT PLUS QU'UN CONFRÈRE, et c'est assumé.
   *
   * Un intervenant qui arrive apporte de l'offre ; un établissement qui
   * arrive apporte de la demande — des renforts à couvrir, des ateliers à
   * programmer, des équipes à former, et cela se répète. C'est la demande
   * qui manque, donc c'est elle qu'on récompense le plus : trois fois un
   * parrainage ordinaire.
   */
  PARRAINAGE_ETABLISSEMENT: 120,
  PUBLICATION: 20,
  ARTICLE: 30,
  MISSION: 50,
  AVIS: 25,
  IDEE: 40,
  /// Répondre à un pair. Conservé pour l'historique des points déjà crédités.
  REPONSE: 15,
...
export class CommunityService {
  ...
  async parrainage(accountId: string) {
    ...
    return {
      ...
      pointsParFilleulEtablissement: BAREME.PARRAINAGE_ETABLISSEMENT,
    };
  }
